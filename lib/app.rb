...
# Choisir 5 handles au hasard et envoyer un tweet
def random_journalists
	...
	# random 5 journalistes
	five_journalists = journalists.sample(5)
	# send a tweet to the 5 handles
	five_journalists.each do |name|
		login_twitter.update ("Hi #{name}, I'm doing a test for my school. Have a nice day ! #bonjour_monde")
	end
end
# récupérer les tweets ayant #bonjour_monde
def tweet_bonjour
	login_twitter.search('#bonjour_monde', result_type: "recent").take(25).each do |tweet|
		puts tweet.text
	end
end
# récupérer les tweets ayant #bonjour_monde et les liker
def user_like
	login_twitter.search('#bonjour_monde', result_type: "recent").take(25).collect do |i|
		puts login_twitter.favorite(i.id)
	end
end
# récupérer les tweets ayant #bonjour_monde et les follow
def usser_folllow
	login_twitter.search('#bonjour_monde', result_type: "recent").take(20).each do |tweet|
		puts login_twitter.follow(tweet.user)
	end
end
#fav stream
def fav_stream
	login_stream.filter(track: "#bonjour_monde") do |tweet|   
